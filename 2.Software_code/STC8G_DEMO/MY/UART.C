...
u8 	xdata GPS_DATA[COM_TX1_Lenth];
u8 Flag_GPS_OK;
#endif
#ifdef UART2
COMx_Define	COM2;
...
void UART1_int (void) interrupt UART1_VECTOR
{
	...
	if(RI)
	{
		...
		if(COM1.B_RX_OK == 0)
		{
			...
			if(temp==0x7F)//连续收到7f触发软件复位，启动自动下载  软件上必须选择最低最高波特率都为115200
			{
				count++;
				if(count>9)
				{
...
//	COMx_InitStructure.UART_BRT_Use   = BRT_Timer2;			//选择波特率发生器, BRT_Timer2 (注意: 串口2固定使用BRT_Timer2, 所以不用选择)
	...
	COMx_InitStructure.UART_RxEnable  = ENABLE;				//接收允许,   ENABLE或DISABLE
	COMx_InitStructure.UART_Interrupt = ENABLE;				//中断允许,   ENABLE或DISABLE
	COMx_InitStructure.UART_Priority    = Priority_1;			//指定中断优先级(低到高) Priority_0,Priority_1,Priority_2,Priority_3
